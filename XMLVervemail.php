...
function getSubscribers( $date_joined_start ,$date_joined_end ){ // It uses an asynchronous mode , so the response must be gotten using utilities_getTasks
     $this->createTagMethodNameNoEmail("legacy.retrieve_active");
     $node = $this->xml->createElement("asynchronous","1");
     $this->methodCall->appendChild($node);
     $node = $this->xml->createElement("date_joined_start","$date_joined_start");
     $this->methodCall->appendChild($node);
     $node = $this->xml->createElement("date_joined_end","$date_joined_end");
     $this->methodCall->appendChild($node);
